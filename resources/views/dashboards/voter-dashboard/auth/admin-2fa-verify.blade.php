    ...
    <div class="w-full max-w-md">
        <div class="bg-white rounded-2xl shadow-xl p-8 border border-gray-300">
            ...
            <div class="text-center mb-6">
                <h1 class="text-2xl font-bold text-gray-900 mb-2">Student Verification</h1>
                <p class="text-sm text-gray-600">
                    A 6-digit security code was sent to
                    @if($hasEmail)
                        <span class="font-bold text-gray-800">{{ $voter->email }}</span>
                    @elseif($hasPhone)
                        <span class="font-bold text-gray-800">{{ $voter->phone_number }}</span>
                    @else
            ...
            <form method="POST" action="{{ route('voter.2fa.verify') }}" class="space-y-5">
                @csrf
                <div class="space-y-2">
                    ...
                    <input type="text" name="code" required autofocus
                           ...
                           maxlength="6"
                           class="block w-full h-14 rounded-xl border-gray-300 shadow-sm text-center text-2xl font-black tracking-widest focus:border-blue-600 focus:ring-blue-600"/>
                    @error('code')
                ...
                <button type="submit"
                        class="w-full h-12 bg-blue-600 text-white text-base font-semibold rounded-lg hover:bg-blue-700 transition shadow-md active:scale-95">
                    Verify and Continue Voting
                </button>
            </form>

            <form method="POST" action="{{ route('voter.2fa.resend') }}" class="mt-4 text-center">
                @csrf
